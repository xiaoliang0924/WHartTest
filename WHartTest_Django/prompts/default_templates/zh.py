...
请开始执行测试。''',
    ...
    },
    {
      ...
      **凭据使用规则**：
      ...
      - 非登录专项用例：步骤从登录后的第一个业务操作开始；若必须写登录，只写成 1 步
      - 严禁使用占位符，必须填写实际凭据值
      ...
      **登录步骤规则（必须遵守）**：
      - 普通功能/展示/权限用例：登录只写在前置条件，步骤不要再拆登录
      - 只有专门测登录的用例，才在步骤里写登录操作
      - 即使写登录，也只能是 1 步，例如：“使用账号17670400361/000000登录系统(http://test.bot.by56.com/work-order/login)” / “登录成功进入系统”
      ...
      复核时：登录不要再拆；业务操作若还能拆成独立动作，再拆开保存。
