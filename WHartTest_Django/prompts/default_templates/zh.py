...
请开始执行测试。''',
    ...
    },
    {
      ...
      **复核重点**：
      ...
      - 前置条件是否含真实账号/密码/URL 和必要业务数据状态
      - 步骤是否原子化：每步只做一个业务操作，且有可判定预期结果；登录不得拆成打开URL/输入账号/输入密码/点击登录
      ...
      **凭据使用规则**：
      ...
      - 登录是前置准备，不要拆成打开URL / 输入账号 / 输入密码 / 点击登录四步
      - 需登录的功能/展示/权限用例：步骤第 1 步必须是登录，且只写 1 步
      - 严禁使用占位符，必须填写实际凭据值
      ...
      **前置条件与步骤粒度（强约束）**：
      ...
      每一步必须是**一个可单独执行的界面或接口操作**，并且必须有**可判定的预期结果**。

      **登录步骤规则（必须遵守）**：
      - 需登录的功能/展示/权限用例：前置条件写清账号，步骤第 1 步再写一次登录
      - 登录只能是 1 步，例如：“使用账号17670400361/000000登录系统(http://test.bot.by56.com/work-order/login)” / “登录成功进入系统”
      ...
      步骤：
      1. 使用李亮账号(admin/admin123)登录系统(http://test.bot.by56.com/work-order/login) / 登录成功进入系统
      2. 在左侧进入该工单详情页，点击“转派” / 弹出转派弹窗
      3. 选择转派目标 / 选择成功
      4. 转派原因保持为空 / 交接说明字段为空
      5. 点击确认转派 / 系统提示“请填写：转派原因”，转派失败
      6. 使用目标账号登录后查看“我的工单” / 该工单不出现在列表中

      复核时：登录只保留 1 步，不要再拆；业务操作若还能拆成独立动作，再拆开保存。
