...
class ResponseObjectBase(object):
    ...
    def __init__(self, resp_obj, parser: Parser):
        ...
        self.parser = parser
        if self.functions_mapping and not parser.functions_mapping:
            parser.functions_mapping = self.functions_mapping
    ...
    def validate(
        ...
        for v in validators:
            ...
            except exceptions.FunctionNotFound as e:
                # 如果函数不存在，记录警告并保留原始表达式
        ...
        # Store validation result without raising exception
        self.validation_results["success"] = validate_pass
        if not validate_pass:
            failures_string = "\n".join([failure for failure in failures])
            self.validation_results["failures"] = failures_string
