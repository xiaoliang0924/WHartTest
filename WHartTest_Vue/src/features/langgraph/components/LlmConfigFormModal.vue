...
const ensureModelsForSlot = async (slotKey: LlmModuleKey) => {
  if (loadingModels.value) {
    return;
  }
  await fetchAvailableModels(slotKey, { silent: true });
};

const validateSlot = (slot: LlmBundleSlot, label: string) => {
  if (!isSlotConfigured(slot.slot_key) && slot.slot_key !== 'llm_chat') {
    return null;
  }
  if (!slot.provider) return text.value.selectProvider(label);
  if (!slot.name.trim()) return text.value.modelNameRequired(label);
  if (!slot.api_url.trim()) return text.value.apiUrlRequired(label);
  if (!/^https?:\/\//.test(slot.api_url.trim())) return text.value.apiUrlInvalid(label);
  return null;
};

const validateForm = () => {
  if (!formData.value.config_name.trim()) {
    return text.value.configNameRequired;
  }
  if (formData.value.is_global && !formData.value.is_active) {
    return text.value.globalShareNeedsActive;
  }
  for (const tab of moduleTabs.value) {
    const error = validateSlot(currentSlot(tab.key), tab.label);
    if (error) return error;
  }
  return null;
};

const buildSubmitSlots = () => {
  return formData.value.slots.map((slot) => {
    const configured = slot.slot_key === 'llm_chat' ? true : isSlotConfigured(slot.slot_key);
    if (!configured) {
      return {
        ...slot,
        slot_label: getModuleTabMeta(slot.slot_key).label,
        is_configured: false,
        provider: '',
        name: '',
        api_url: '',
        ...
        enable_hitl: true,
        enable_streaming: true,
        default_runtime_mode: slot.default_runtime_mode || 'auto',
      };
    }
...
const handleSubmit = () => {
  ...
  emit('submit', {
    ...formData.value,
    slots: buildSubmitSlots(),
  }, props.configData?.id);
};

const handleTestCurrentSlot = async () => {
  if (!props.configData?.id) {
    Message.warning(text.value.saveBeforeTest);
    return;
  }
  testingSlot.value = true;
  try {
    const response = await testLlmBundleSlot(props.configData.id, activeTab.value);
    if (response.status === 'success') {
      Message.success(response.message ? tl(response.message) : text.value.testSuccess);
    } else {
      Message.error(response.message ? tl(response.message) : text.value.testFailed);
    }
  } catch (error: any) {
    if (!isActiveTestRequest(sessionId, requestId)) {
      return;
